...
from boat_ms.utils.op_utils import update_tensor_grads, conjugate_gradient


class CG_IAD_PTT(HyperGradient):
    """
    Calculation of the hyper gradient of the upper-level variables with Conjugate Gradient (CG) _`[1]`,
    Initialization-based Auto Differentiation (IAD) _`[2]` and Pessimistic Trajectory Truncation (PTT) _`[3]`.
    ...
    _`[3]` Liu R, Liu Y, Zeng S, et al. Towards gradient-based bilevel optimization
    ...
    def __init__(
        ...
        super(CG_IAD_PTT, self).__init__(ul_objective, ul_model, ll_model, ll_var, ul_var)
    ...
    def compute_gradients(
        ...
        if self.gda_loss is not None:
            ...
            lower_loss = self.gda_loss(ll_feed_dict, ul_feed_dict, self.ul_model, auxiliary_model, params=lower_model_params)
        else:
