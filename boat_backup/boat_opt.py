...
import boat_ms.higher_ms as higher
from boat_ms.utils.op_utils import copy_parameter_from_list, average_grad
try:
...
import importlib
ll_grads = importlib.import_module("boat_ms.dynamic_ol")
ul_grads = importlib.import_module("boat_ms.hyper_ol")
    ...
    def build_ll_solver(self, lower_opt: Optimizer):
        ...
        if self.boat_configs['fo_gm'] is None:
            ...
            if 'DM' in self._dynamic_op:
                print("right!")
                self.boat_configs["DM"]['auxiliary_v'] = [ops.zeros_like(param) for param in self._ll_var]
                self.boat_configs["DM"]['auxiliary_v_opt'] = nn.SGD(self.boat_configs["DM"]['auxiliary_v'],
                                                                     learning_rate=self.boat_configs["DM"][
                                                                         'auxiliary_v_lr'])
