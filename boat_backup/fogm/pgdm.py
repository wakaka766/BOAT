...
from boat_ms.utils.op_utils import grad_unused_zero, require_model_grad, update_tensor_grads, stop_model_grad
...
class PGDM(DynamicalSystem):
    ...
    def optimize(
        ...
        def loss_fn():
            ...
            ll_loss_current = self.ll_objective(ll_feed_dict, self.ul_model, self.ll_model)
        ...
        def compute_and_update_grads(loss_fn, ll_model, ul_model, ll_opt, ul_opt):
            grad_fn_ll = ops.GradOperation(get_by_list=True)(loss_fn, ll_model.trainable_params())
            ll_grads = grad_fn_ll()
            ll_opt(ll_grads)
            grad_fn_ul = ops.GradOperation(get_by_list=True)(loss_fn, ul_model.trainable_params())
            ul_grads = grad_fn_ul()
            ul_opt(ul_grads)

        compute_and_update_grads(loss_fn, self.ll_model, self.ul_model, self.ll_opt, self.ul_opt)
