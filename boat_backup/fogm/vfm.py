...
from boat_ms.utils.op_utils import update_grads, grad_unused_zero, require_model_grad, update_tensor_grads, stop_model_grad
...
class VFM(DynamicalSystem):
    ...
    def optimize(
        ...
        y_hat_opt = nn.SGD(y_hat.trainable_params(), learning_rate=self.y_hat_lr, momentum=0.9)
        ...
        def g_x_xhat_w(y, y_hat, x):
            ...
            grad_y = ops.GradOperation(get_by_list=True)(self.ll_objective, y.trainable_params())(
                ll_feed_dict, x, y)
            grad_x = ops.GradOperation(get_by_list=True)(self.ll_objective, x.trainable_params())(
                ll_feed_dict, x, y)
        ...
        for y_itr in range(self.lower_loop):
            ...
            grad_fn = ops.GradOperation(get_by_list=True)(self.ll_objective, y_hat.trainable_params())
        ...
        grad_F_y = ops.GradOperation(get_by_list=True)(self.ul_objective, self.ll_model.trainable_params())(
            ul_feed_dict, self.ul_model, self.ll_model)
        grad_F_x = ops.GradOperation(get_by_list=True)(self.ul_objective, self.ul_model.trainable_params())(
            ul_feed_dict, self.ul_model, self.ll_model)
        ...
        delta_F[:n_params_y] = mnp.concatenate([p.view(-1) for p in grad_F_y]).astype(ms.float32)
        delta_f[:n_params_y] = mnp.concatenate([p.view(-1) for p in gy]).astype(ms.float32)
        delta_F[n_params_y:] = mnp.concatenate([p.view(-1) for p in grad_F_x]).astype(ms.float32)
        delta_f[n_params_y:] = mnp.concatenate([p.view(-1) for p in gx_minus_gx_k]).astype(ms.float32)

        norm_dq = (ops.norm(delta_f) ** 2).astype(ms.float32)  # 使用 ops.norm 替代 mnp.linalg.norm
