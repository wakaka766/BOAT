...
from boat_ms.utils.op_utils import update_grads, grad_unused_zero, update_tensor_grads, copy_parameter_from_list
import numpy as np
...
class MESM(DynamicalSystem):
    ...
    def __init__(
        ...
        self.y_hat_opt = nn.SGD(self.y_hat.trainable_params(), learning_rate=solver_config['MESM']['y_hat_lr'],
                                momentum=0.9)
    ...
    def optimize(
        ...
        grad_theta_parameters = grad_fn(self.ll_objective, self.y_hat.trainable_params())(ll_feed_dict, self.ul_model,
                                                                                          self.y_hat)

        errs = []
        for a, b in zip(list(self.y_hat.trainable_params()), list(self.ll_model.trainable_params())):
            diff = a - b
            errs.append(diff)
        vs_param = []
        for v0, gt, err in zip(list(self.y_hat.trainable_params()), grad_theta_parameters, errs):
        ...
        lower_loss = (1 / ck) * self.ul_objective(ul_feed_dict, self.ul_model, self.ll_model) + \
                     self.ll_objective(ll_feed_dict, self.ul_model, self.ll_model) - 0.5 * self.gamma_1 * reg
        ...
        grad_y_parameters = grad_fn_lower(wrapped_ll_objective, self.ll_model.trainable_params())()
        ...
        def wrapped_ul_objective():
            return (1 / ck) * self.ul_objective(ul_feed_dict, self.ul_model, self.ll_model) + \
        ...
        grad_x_parameters = grad_fn_upper(wrapped_ul_objective, self.ul_model.trainable_params())()
