...
class TestSystemFeature:
    ...
    def test_few_shot(self, learn_lr, use_t, use_warp, use_forget):
        backbone_model = model.backbone.Conv([5, 3, 84, 84], 5, num_filters=32, use_head=True)
        meta_model = boat.utils.model.MetaModel(backbone_model, learn_lr=learn_lr, use_t=use_t,
                                                 use_warp=use_warp, use_forget=use_forget)
        outer_opt = torch.optim.Adam(meta_model.parameters())

        optimizer = boat.optimizer.Problem("Initial", inner_objective=inner_o,
    ...
    def test_regular(self, learn_lr, use_t, use_warp, use_forget):
        backbone_model = model.backbone.Conv([32, 3, 84, 84], 8, num_filters=32, use_head=True)
        meta_model = boat.utils.model.MetaModel(backbone_model, learn_lr=learn_lr, use_t=use_t,
                                                 use_warp=use_warp, use_forget=use_forget)
        outer_opt = torch.optim.Adam(meta_model.parameters())
