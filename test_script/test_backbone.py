...
class TestBackbone:
    ...
    def test_bilevel_model_conv4(self):
        ul_model = model.backbone.Conv([32, 3, 84, 84], 8, num_filters=32, use_head=False)
    ...
    def test_bilevel_model_res12(self):
        ul_model = model.backbone.Res12([32, 3, 84, 84], 8, use_head=False)
    ...
    def test_singlelevel_model_conv4(self):
        model = model.backbone.Conv([32, 3, 84, 84], 8, num_filters=32, use_head=True)
        model(tr_xs)

    def test_singlelevel_model_res12(self):
        model = model.backbone.Res12([32, 3, 84, 84], 8, use_head=True)
        model(tr_xs)
