...
def get_sinuoid():
    fc_net = nn.Sequential(nn.Sequential(nn.Linear(1, 64), nn.ReLU(inplace=True), nn.LayerNorm(normalized_shape=64)), nn.Linear(64, 64), nn.ReLU(inplace=True),
                                nn.LayerNorm(normalized_shape=64),nn.Sequential(nn.Linear(64,1)))
    initialize(fc_net)
    return fc_net
...
def main():
    ...
    with tqdm(dataloader, total=10, desc="Meta Training Phase") as pbar:
        for meta_iter, batch in enumerate(pbar):
            ul_feed_dict = [{"data": batch["test"][0][k].float().to(device), "target": batch["test"][1][k].float().to(device)} for k in range(batch_size)]
            ll_feed_dict = [{"data": batch["train"][0][k].float().to(device), "target": batch["train"][1][k].float().to(device)} for k in range(batch_size)]
            print(ll_feed_dict['data'].shape,ll_feed_dict['target'].shape)
