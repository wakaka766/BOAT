...
from boat.ul_solver.iad import IAD
...
ll_grads = importlib.import_module("boat.ll_solver")
ul_grads = importlib.import_module("boat.ul_solver")
...
class Problem(object):
    ...
    meta_model: Module, default=None  # todo
        ...
        but if choose other derived methods, internally defined boat.utils.adapt_model.meta_model should be used
        ...
        if self._method == "Feature":
            ...
            if isinstance(meta_model, MetaModel):
                ...
                raise TypeError(
                    ...
                    ' be implemented with model wrapper defined by boat.utils.model.meta_model.MetaModel'
                )
    ...
    def build_ll_solver(
        ...
        """
        Build LL-problem solver with boat.lower_optimizer module,
        ...
        method module of boat.ul_solver
        ...
        ----------
            ...
            ll_l2_reg: float, default=0.1
                ...
                function of the regularizedLL problem. Referring to module boat.ul_solver.bvfim
                for more details.
            ...
            ul_l2_reg: float, default=0.01
                ...
                boat.ul_solver.bvfim for more details.

            ul_ln_reg: float, default=10.
                Specify parameter for BVFIM method. Weight of the log-barrier penalty term in the
                value function of the regularized UL problem. Referring to module boat.ul_solver.bvfim
                for more details.
        ...
        if self._ll_method == 'Dynamic':
            if self._ul_method == 'FD':
                assert lower_loop == 1, "Finite Differentiation method requires one gradient step to optimize task parameters."
    ...
    def build_ul_solver(
        ...
        ----------
            ...
            r: float, default=1e-2
                ...
                boat.ul_solver.onestage for more details.
        ...
        if self._ll_method == "Dynamic":
            ...
            kwargs['ll_objective'] = self._ll_objective
            if self._ul_method == "CG" or self._ul_method == "NS":
                kwargs['lower_learning_rate'] = self._lower_learning_rate
                kwargs['k'] = k
                kwargs['tolerance'] = tolerance
        elif self._ul_method == "VSM":
            kwargs['ll_objective'] = self._ll_objective
            kwargs['ll_l2_reg'] = ll_l2_reg
            kwargs['ul_l2_reg'] = ul_l2_reg
            kwargs['ul_ln_reg'] = ul_ln_reg
    ...
    def build_meta_solver(
        ...
        self._meta_problem_solver = IAD(self._meta_model, self._inner_objective,
        ...
        for t_idx, (train_x, train_y, val_x, val_y) in enumerate(zip(train_data_batch, train_target_batch,
            ...
            if self._method == "Initial":
                ...
                if self._ll_method == "BVFIM":
                    ...
                    out, auxiliary_model = self._ll_solver.optimize(train_x, train_y, auxiliary_model, auxiliary_opt,
                                                                    val_x, val_y, reg_decay)
                    # val_acc += accuary(out, val_y) / 4
                    forward_time = time.time() - forward_time
                    backward_time = time.time()
                    loss = self._ul_solver.compute_gradients(val_x, val_y, auxiliary_model, train_x,
                                                             train_y, reg_decay)
                    ...
                    with higher.innerloop_ctx(self._ll_model, self._lower_opt,
                        ...
                        pmax = self._ll_solver.optimize(train_x, train_y, auxiliary_model, auxiliary_opt,
                                                        **kwargs_lower)
                        forward_time = time.time() - forward_time
                        ...
                        if self._ll_method == "Dynamic":
                            ...
                            kwargs_upper['train_data'] = train_x
                            kwargs_upper['train_target'] = train_y
                        loss = self._ul_solver.compute_gradients(val_x, val_y, auxiliary_model, **kwargs_upper)
                        backward_time = time.time() - backward_time
