        ...
        def check_model_structure(model1, model2):
            for param1, param2 in zip(model1.parameters(), model2.parameters()):
                ...
                if param1.device != param2.device:
                    return False
            return True
        assert check_model_structure(self.ll_model, self.ul_model), "With 'IAD' or 'FOA' operation, the 'ul' model must have the same structure as the 'll_model', and the 'll variables' should initialize to the 'ul variables'. "
        ul_loss = self.ul_objective(ul_feed_dict, self.ul_model, auxiliary_model)
        grads_upper = torch.autograd.grad(ul_loss, list(auxiliary_model.parameters(time=0)), allow_unused=True)
