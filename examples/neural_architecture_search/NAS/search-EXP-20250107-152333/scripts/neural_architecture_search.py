...
parser.add_argument('--epochs', type=int, default=3, help='num of training epochs')#default=50
...
def main():
    ...
    for epoch in range(args.load_iter, args.epochs):
        ...
        train_acc, train_obj,epoch_time = train(train_queue, valid_queue, model, criterion, optimizer, b_optimizer)
...
def train(train_queue, valid_queue, model, criterion,optimizer, boat_optimizer):
    ...
    for step, (input, target) in enumerate(train_queue):
        ...
        if step % args.report_freq == 0:
            logging.info('train %03d loss: %e top1: %f top5: %f runtime: %f', step, objs.avg, top1.avg, top5.avg, time.avg)
