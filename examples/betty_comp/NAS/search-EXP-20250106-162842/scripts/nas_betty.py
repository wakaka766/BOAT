...
parser.add_argument("--unroll_steps", type=int, default=10, help="unrolling steps")
...
def main():
    ...
    classifier_net = Network(
        args.init_channels, 10, args.layers, criterion, steps=args.arch_steps
    )
    ...
    classifier_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        classifier_optimizer, float(train_iters // args.unroll_steps), eta_min=args.learning_rate_min
    )
